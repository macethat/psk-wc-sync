...
## 2026-08-11 � Mailchimp: revisi�n de suscriptores nuevos + desactivaci�n del doble opt-in

### Contexto
- El usuario pidi� revisar si llegan suscriptores nuevos por el formulario del sitio (MC4WP form 417 -> lista 27b4cb9f8c) y si les llegan los mails.

### Revisi�n (v�a API Mailchimp desde el servidor)
- Lista tiene 13 suscriptores. Solo 2 son nuevos desde la migraci�n (05-ago): henrybatista24@icloud.com (signup 2026-08-11, pending) y xeniayazmin27@gmail.com (signup 2026-08-06, pending).
- Ambos con timestamp_opt vac�o y activity vac�a -> nunca confirmaron el doble opt-in (el correo de confirmacion puede ir a spam; la entrega del opt-in no se expone en la API de membresia, solo se ve que no hubo clic).
- Config form 417: double_optin=1, lists=[27b4cb9f8c], sin automatizaciones ni campa�as en la lista.
...
### Diagnostico
- **Merchant Center**: S� conectado (GLA activo, merchant_id 5825178723, verify/link/claim/sdi_update OK, site verified meta INv26I0qwEqDUhHG74QuIeiH1zBX1mv_ZYaWGnQx9QE en HTML, target PA). El "no visible en codigo" es normal: GLA sincroniza v�a Content API (servidor a servidor), no hay feed XML publico (endpoint REST da 404 intencional).
...
- **Google Ads**: gla_ads_account_state.account_access=-1 "La cuenta debe ser aceptada antes de completar la configuraci�n" + link_ads=PERMISSION_DENIED. El sub-account de Ads fue creado (sub_account=1) pero el invite esta SIN ACEPTAR (billing pendiente). Accion manual en dashboard de Google Ads (gla_ads_billing_url).
...
### Cambio aplicado
- functions.php (filtro rank_math/json_ld): en el Store/Organization del home se a�ade sameAs con la URL de Google Maps de la sucursal El Cangrejo (0x8faca91dc4c6f185...), conservando Facebook. Backup: functions.php.bak-20260813-sameas.
...
- El cron S� se dispara todos los dias, pero estaba FALLANDO desde el 17-08-2026: faltaba ~/wc_export_ssh.php (necesario para el export via wp eval-file). Los ultimos 4 runs (17,18,19,20 ago) terminaron con ERROR WP-CLI + salida vacia.
...
- Creada pagina NUEVA ID 22030: titulo 'T�RMINOS Y CONDICIONES - SUPLEMENTOS PANAM�', slug terminosycondiciones, template default, _elementor_data = container 76adf909 + widget text-editor 3d569a67 con el contenido nuevo (CSS sp-tc-wrap + HTML + media query 767px).
...
- 2026-09-07: revisada imagen del combo 21960 (Creatina VMS + Glutamina VMS) por reporte de 'no se ve'. Causa raiz: la imagen (attachment 21963, subida 11-08) tenia en _wp_attachment_metadata tama�os declarados pero 4 archivos fisicos FALTABAN (-1024x1024, -150x150, -1070x510, -100x100 -> HTTP 404). El srcset del <img> principal incluia esos candidatos rotos; segun el viewport del navegador/dispositivo podia elegir un 404 y la imagen no se mostraba (en Opera del usuario aparecia porque elegia el 800x800 que existia). Regenerado con 'wp media regenerate 21963' -> archivos creados.
- El mismo problema afectaba a otros 2 combos subidos el mismo dia (11-08): 21962 (Primeval+Creatina Angry, attach 21965) y 21961 (Mutant Mass+Creatina Angry, attach 21964). Regenerados ambos.
- Verificado: los 27 combos grouped ahora tienen TODOS sus tama�os de imagen fisicos; URLs antes rotas responden 200. Cache purgada.
- 2026-09-07 (fix checkbox T&C en combos): en desktop el checkbox 'He leido y acepto los Terminos y Condiciones' aparecia AL LADO del boton (y el contenedor crecia de alto) en las fichas de combos grouped. Causa: .woocommerce-grouped-add-to-cart es display:flex en fila (desktop) con order en quantity/boton/wishlist; .sp-tc-agree (order 0) se colaba al lado. Solucion: CSS en sp-tc-agree.php (bloque sp-tc-agree-css en wp_head) -> .woocommerce-grouped-add-to-cart .sp-tc-agree{flex:0 0 100%;order:99;width:100%} fuerza linea propia debajo del boton; los woosw/woosc adyacentes pasan a order 98. Aplica desktop y movil. Verificado: CSS en front (combo 21512), agree despues del boton en desktop y movil, checklist retiro OK. Copia local actualizada.
- 2026-09-07 (2do fix checkbox T&C en combos desktop): el fix anterior (flex:0 0 100%) NO saltaba de linea porque el contenedor .woocommerce-grouped-add-to-cart es display:flex con flex-wrap NOWRAP en desktop. Corregido: a�adido .woocommerce-grouped-add-to-cart{flex-wrap:wrap} para que el .sp-tc-agree (flex:0 0 100%;order:99) ocupe linea propia debajo del boton. Verificado estructura: qty -> boton -> agree -> woosw.
...
- 2026-09-09 (stock 4 productos nuevos desde PSKloud): consultada API PSKloud (/Api/Articulos + /Api/Existencias) desde servidor. Stock por sucursal aplicado a variaciones/simple v�a metas _sucursal_1/5/6/7/8/10_stock + _sucursales_disponibles + _stock + _stock_status. Resultado: EVOGEN Xtreme (79/80/73, instock, todas las sucursales), Fiber (30), ISO Surge Vainilla/Chocolate (31/31), RAW Glycerol (3 sabores en 0 en PSK -> outofstock, sin sucursales). Verificado: ficha de ISO Surge renderiza bloque .sp-sucursal-stock con data-sp-agg/var/status/names correctos (distribucion por sucursal). El stock de RAW Glycerol quedara 0 hasta que PSKloud registre existencias; el cron diario lo actualizara luego.
- Nota: el script del servidor (psk-sync/daily_stock_update.py) NO maneja _sucursal_*; el sync por sucursal se aplico manualmente via API para estos 4. Los 4 productos ya tienen sus metas de sucursal listas para que el cron/checkout las use.

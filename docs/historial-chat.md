...
### Tema
Verificaci�n del script de sync diario (stock + precios) desde el cron del servidor.

### Hallazgos
- El proceso corre en el servidor via cron de SiteGround (02:00), NO desde la m�quina local.
- El cron se dispara a diario pero fallaba desde el 17-08: faltaba ~/wc_export_ssh.php (export auxiliar para wp eval-file).
- Repuesto el archivo desde local/wc_export_ssh.php; dry-run del flujo completo OK (704 art�culos PSK ? 387 productos WC ? 163 a cambiar).

### Acciones
- Restaurado ~/wc_export_ssh.php en el servidor.
- Actualizado docs/proceso_actualizacion_diaria.md con secci�n Troubleshooting del cron.
- Documentado en historial-chat.md ra�z y docs/historial-cambios.md.
